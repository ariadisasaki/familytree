...
function buildTree(data) {
  ...
  data.forEach(p => {
    ...
    // Jika anak hanya punya ayah (single parent)
    else if (p.ayah_id) {
      let key = p.ayah_id;
      if (!couples[key]) {
        couples[key] = {
    ...
    // Jika anak hanya punya ibu (single parent)
    else if (p.ibu_id) {
      let key = p.ibu_id;
      if (!couples[key]) {
        couples[key] = {
...
function drawTree(treeData) {
  ...
  const svg = d3.select("#tree")
    ...
    .attr("height", height)
    .append("g")
    .attr("transform", "translate(50,20)"); // margin kiri & atas

  const root = d3.hierarchy(treeData);

  const treeLayout = d3.tree().size([width - 100, height - 100]); // horizontal = width, vertikal = height
  treeLayout(root);
